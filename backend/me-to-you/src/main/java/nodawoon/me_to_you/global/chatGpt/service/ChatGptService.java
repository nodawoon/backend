...
//    private final String questionPrompt = "답변은 주어진 정보에 기반하여 직접적으로 제공해줘. 그리고 사실 기반으로 정확한 정보만 알려주고 해당 질문에 대해서만 답변하고 다른 이상한 내용은 답변하지 말아줘"
//            + "제공한 정보에 대해서 질문에 대한 답변 정보가 부족할 경우 애매하게 너가 다변하지 말고'잘 모르겠어요'라고 명시하세요." ;
    ...
    private final String questionPrompt = "안녕! 난 네 친한 친구 같은 AI야. 나에 대한 정보가 위에 질문과 답변으로 주어져 있어. 그 정보만을 바탕으로 네 질문에 정확하게 대답할게. 편하게 물어봐!\n" +
            ...
            "3. 위에 있는 정보로 대답할 수 있는 질문이면, 친구한테 말하듯이 편하게 대답할 거야. 예를 들어, '너의 비밀 알려줘'라고 물어보면 '어, 내 비밀? 사실 나는 탈모야.'라고 정확히 대답할 거야.\n" +
            "4. " + LocalDate.now() + " 이 시간을 기준으로 답변해줄게. 나이, 기간, 날짜 관련 질문은 이 날짜를 기준으로 정확히 계산해서 대답할 거야.\n" +
            "5. 주어진 정보에 직접적으로 관련된 질문에는 반드시 그 정보를 사용해서 대답할 거야. 예를 들어, '비밀'에 대해 물어보면 주어진 비밀 정보를 답변할 거야.\n" +
    ...
    public String generateChatgpt(String selfSurveyPrompt, String chatBotPrompt, String userProfilePrompt, String question) {
        String message = selfSurveyPrompt + chatBotPrompt + userProfilePrompt
                + "위의 문답을 보고 다음 질문에 대답해줘! 답변은 너의 입장에서 답변을 친한 친구한테 하는 것처럼 해주는데 나의 의견은 절대로 다시 물어보지마."
                + questionPrompt
                + question;
